...
$sercatid = $_GET['sercatid'] ?? 0;

// Fetch company details
$mainCategorySql = "SELECT * FROM ind_maincat WHERE sercatid = ?";
$mainCategoryStmt = $conn->prepare($mainCategorySql);
$mainCategoryStmt->execute([$sercatid]);
...
<div class="container mt-4 ">
    <h5 class="text-start"><?= htmlspecialchars($mainCategory['main_cat']) ?></h5>
    <p class="text-start"><?= htmlspecialchars($mainCategory['descript'] ?? 'No description available') ?></p>
    <hr />
    <div class="p-2" style="background-color: #fff; box-shadow: 0 2px 4px 0 rgba(0,0,0,0.2), 0 3px 16px 0 rgba(0,0,0,0.19); border-radius: 10px;">
        ...
        <div style="display: flex; gap: 10px; flex-wrap: wrap; justify-content: start; align-items: start; align-content: start;">
            <?php
            $serviceSql = "SELECT * FROM ind_subcat WHERE main_id = ?";
            $serviceStmt = $conn->prepare($serviceSql);
            $serviceStmt->execute([$mainCategory['sercatid']]);
            ...
            ?>
                    <div class="service-card"
                        ...
                        <p class="card-text text-truncate" style="margin-top: -5px; max-width: 150px;">
                            <?= htmlspecialchars(strlen($service['sub_categor']) > 10 ? substr($service['sub_categor'], 0, 10) . '...' : $service['sub_categor']) ?></p>
                    </div>
            <?php
                }
            } else {

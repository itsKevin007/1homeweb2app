...
function update_data()
{
	...
	$contact = isset($_POST['contactNum']) ? $_POST['contactNum'] : '';
		...
		$sql = $conn->prepare("UPDATE tbl_management SET contactNum = :contact, last_modified = :today_date1, modified_by = :modified_by");
		$sql->bindParam(':contact', $contact);
		$sql->bindParam(':today_date1', $today_date1, PDO::PARAM_STR);
		$sql->bindParam(':modified_by', $userId, PDO::PARAM_INT);
		$sql->execute();

		$log = $conn->prepare("INSERT INTO tr_log (module, action, description, log_action_date, action_by) VALUES ('Content Management', 'Contact Modify', 'EDIT: $contact', '$today_date1', '$userId')");
		$log->execute();
		
		echo '<form id="redirectForm" action="'. ADM_ROOT .'settings/" method="POST">

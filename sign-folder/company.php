...
$companyName = isset($_POST['firstName']) ? trim($_POST['firstName']) : '';
...
// echo " Name: $companyName<br>";
// echo "Email Address: $emailadd<br>";
// echo "Contact Number: $connum<br>";
...
if($chk->rowCount() > 0)
{
    header('Location: ../sign-up.php?mail=exist');              
		...
		$bsUser = $conn->prepare("INSERT INTO bs_user (firstname, email, username, contactno, password, access_level) 
												VALUES (:fname, :emailadd, :emailadd, :connum, md5(:password), '1')");
		$bsUser->bindParam(':fname', $companyName, PDO::PARAM_STR);
		...
		$sql = $conn->prepare("INSERT INTO tbl_company (user_id, bname, emailadd, connum,
								 in_region, in_prov, in_city, in_barangay, date_added, added_by) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

        // Execute the statement with the collected values
        $sql->execute([
            $userId, $companyName, $emailadd, $connum, 
        ...
        // Update the unique id in the database
        $sql1 = $conn->prepare("UPDATE tbl_company SET uid = ? WHERE com_id = ?");
        $sql1->execute([$uid1, $id1]);

        // Send auto-reply email to the client
        $to = $emailadd;
        $subject = "One Home Solutions PH - Company Service Provider";
        $txt = "
        Dear <b>$companyName</b>,<br /><br />
        Thank you for Subscribing to One Home as a Company Service Provider. This is to confirm that we have received your email.
		...
		$subject1 = "Notification - Company Service Provider";
		$txt1 = "
		".$today_date1." <br />
		<br />
		Subject: New Service Provider Company<br /> <br />
	
		Company Name: <b>" . $companyName . ".</b><br />
		<b>Contact Information: <b>" . $emailadd . ".</b><br />
		<b>Contact Number:<b> " . $connum . ". </b><br /> <br />
		<b>Subscription Type:<b> Company Service Provider </b><br />
